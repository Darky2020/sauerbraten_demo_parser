...
def packet_dict(packet):
    ...
    if packet.type == N_DEMOPACKET:
    ...
    elif packet.type == N_EDITF or\
                 packet.type == N_EDITT or\
                 packet.type == N_EDITM or\
                 packet.type == N_FLIP or\
                 packet.type == N_COPY or\
                 packet.type == N_PASTE or\
                 packet.type == N_ROTATE or\
                 packet.type == N_REPLACE or\
                 packet.type == N_DELCUBE or\
                 packet.type == N_EDITVSLOT:
        ...
        for i in range(args[2]):
            ...
            if packet.context["current_mode"] in [m_hold, m_insta_hold, m_effic_hold]:
        ...
        if flags&(1<<4):
            ...
            if flags&(1<<6):
                ...
                if clamp(dir/360, 0, 180) - 90:
                    falling["x"] *= math.cos(math.radians(clamp(dir/360, 0, 180) - 90))
                    falling["y"] *= math.cos(math.radians(clamp(dir/360, 0, 180) - 90))
                    falling["z"] =  math.sin(math.radians(clamp(dir/360, 0, 180) - 90))
